...
const AllAds = ({ user }) => {
    ...
    useEffect(() => {
        const fetchAds = async () => {
            ...
            } catch (err) { console.error("Ads fetch error"); }
        };
        fetchAds();
    }, []);

    // Fetch Seller Stats & Reviews from Database
    useEffect(() => {
        const fetchSellerDetails = async () => {
            ...
            try {
                ...
                const res = await axios.get(`http://localhost:8000/api/reviews/seller/${uid}`);
                
                // Backend returns { seller: {}, reviews: [] }
    ...
    const startChat = async (isReport = false) => {
        ...
        try {
            const res = await axios.post("http://localhost:8000/api/chat/start", {
                buyerId: user.uid, sellerId: selectedAd.posted_by_uid, adId: selectedAd._id
            }, { headers: { Authorization: `Bearer ${user.accessToken}` } });
            if (res.data.chatId) navigate(`/chat/${res.data.chatId}`, { state: { isReportRequest: isReport } });
        } catch (error) { toast.error("Chat start nahi ho saki."); }
    };

    return (
        <div className="w-full min-h-screen bg-gray-50 pb-10">
            {/* 1. Header & 14 Filters (Original Design) */}
            <div className="p-4 md:p-8 bg-white border-b shadow-sm">
                ...
                <div className="flex space-x-2 overflow-x-auto pb-2 no-scrollbar">
                    {MAIN_CATEGORIES.map((cat) => (
                        <button key={cat.code} onClick={() => setActiveCategory(cat.code)}
            ...
            {/* 3. Ad Detail Modal (Dynamic Data from Schema) */}
            {selectedAd && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-[100] p-2 md:p-10">
                    <div className="bg-white w-full h-full md:h-auto md:max-h-[90vh] md:max-w-6xl md:rounded-[2rem] shadow-2xl relative flex flex-col overflow-hidden">
                        <button onClick={() => setSelectedAd(null)} className="absolute top-5 right-5 text-3xl text-gray-400 hover:text-red-500 z-10">&times;</button>
                        ...
                        <div className="overflow-y-auto p-4 md:p-8 flex-1">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10">
                                <div className="relative h-64 md:h-[450px] rounded-2xl overflow-hidden border bg-gray-50">
                                    <img src={selectedAd.images[currentImageIndex]} className="w-full h-full object-contain" alt="" />
                                    {selectedAd.images?.length > 1 && (
                                        <div className="absolute inset-0 flex items-center justify-between px-2">
                                            <button onClick={() => setCurrentImageIndex(p => (p - 1 + selectedAd.images.length) % selectedAd.images.length)} className="bg-white/80 p-2 rounded-full text-pink-600 shadow"><FaArrowLeft/></button>
                                            <button onClick={() => setCurrentImageIndex(p => (p + 1) % selectedAd.images.length)} className="bg-white/80 p-2 rounded-full text-pink-600 shadow"><FaArrowRight/></button>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col justify-between">
                                    <div>
                                        ...
                                        <div className="flex items-center text-gray-500 text-xs mt-4"><FaMapMarkerAlt className="mr-2 text-pink-500" /> {selectedAd.location}</div>
                                        ...
                                        <div className="mt-6 border-t pt-4">
                                            ...
                                            <div className="grid grid-cols-2 gap-3 text-sm italic text-gray-700">
                                                ...
                                                {Object.entries(selectedAd.details || {}).map(([key, val]) => (
                                                    <p key={key} className="capitalize"><b>{key}:</b> {val}</p>
                                                ))}
                                        ...
                                        <div className="mt-8 p-5 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
                                            <div className="flex justify-between items-center">
                                                ...
                                                <button onClick={() => setProfileModalUid(selectedAd.posted_by_uid)} className="bg-white border-2 border-pink-100 text-pink-600 px-4 py-2 rounded-xl font-bold text-[10px] uppercase shadow-sm hover:bg-pink-50">View Profile</button>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="mt-8 space-y-3">
                                        <button onClick={() => startChat(false)} className="w-full bg-green-500 text-white py-4 rounded-2xl font-black text-lg shadow-lg active:scale-95 transition">Inquire Now</button>
                                        <button className="w-full bg-blue-500 text-white py-4 rounded-2xl flex items-center justify-center font-black text-lg shadow-lg"><FaPhoneAlt className="mr-3" /> Contact Seller</button>
                                        <button onClick={() => startChat(true)} className="w-full text-red-400 text-[10px] font-bold flex items-center justify-center gap-1 uppercase opacity-60 hover:opacity-100 transition"><FaFlag /> Report Ad</button>
                                    </div>
                                </div>
                            </div>
            ...
            {/* 4. PREMIUM PROFILE MODAL (Real Database Data) */}
            {profileModalUid && (
                <div className="fixed inset-0 bg-black/90 z-[120] flex items-center justify-center p-4 backdrop-blur-md">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] overflow-hidden relative flex flex-col max-h-[85vh] shadow-2xl animate-in slide-in-from-bottom duration-300">
                        <div className="bg-pink-600 p-8 text-white text-center relative">
                            <button onClick={() => setProfileModalUid(null)} className="absolute top-5 right-6 text-white text-2xl">&times;</button>
                            <div className="relative inline-block">
                                <FaUserCircle size={80} className="mx-auto mb-3 bg-white text-pink-100 rounded-full shadow-lg" />
                                <div className="absolute bottom-4 right-0 bg-blue-500 p-1 rounded-full border-2 border-white"><FaCheckCircle className="text-white" size={12} /></div>
                            </div>
                            ...
                            <div className="flex items-center justify-center gap-2 mt-1 opacity-80">
                                <span className="bg-white/20 px-3 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider flex items-center gap-1"><FaShieldAlt/> Verified Merchant</span>
                            </div>
                        </div>
                                        ...
                                        <div key={rev._id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                <div className="flex text-yellow-400 text-[10px]">
                                                    {[...Array(5)].map((_, i) => <FaStar key={i} className={i < rev.rating ? "text-yellow-400" : "text-gray-200"} />)}
                                                </div>
                                            </div>
