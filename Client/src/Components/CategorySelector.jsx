...
const CategorySelector = ({ onClose, onCategorySelect }) => {
    return (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[100] p-4">
            {/* Modal Container */}
            <div className="bg-white p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-2xl transform transition-all duration-300 scale-100">
                
                <h2 className="text-3xl font-extrabold text-pink-600 mb-6 border-b pb-3">
                    What are you listing?
                </h2>
                
                {/* Categories Grid - 3 Columns on Medium Screens */}
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 max-h-96 overflow-y-auto pr-2">
                    {categories.map((cat) => (
                        <button
                            key={cat.code}
                            ...
                            className="flex flex-col items-center p-4 border border-gray-200 rounded-xl transition duration-300 
                                       hover:bg-pink-50 hover:border-pink-500 hover:shadow-md group"
                        >
                            ...
                            {/* Name */}
                            <span className="text-sm font-semibold text-center text-gray-800 group-hover:text-pink-700 transition-colors">
                                {cat.name}
                            </span>
                        </button>
                    ))}
                ...
                <button
                    onClick={onClose}
                    className="mt-6 w-full py-3 bg-gray-100 text-gray-700 font-medium rounded-xl hover:bg-gray-200 transition duration-300"
                >
                    Cancel
                </button>
            </div>
