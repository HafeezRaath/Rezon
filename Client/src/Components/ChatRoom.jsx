...
const ChatRoom = ({ user }) => {
    ...
    const [adPreviewModal, setAdPreviewModal] = useState(null); // 🆕 Detailed Preview State
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            try {
                const res = await axios.get(`${API_BASE_URL}/api/chat/history/${conversationId}`, {
    ...
    const submitReview = async () => {
        ...
        if (!adIdToSubmit) return toast.error("Invalid Ad link.");
        try {
            await axios.post(`${API_BASE_URL}/api/reviews`, { sellerId: otherUser.uid, adId: adIdToSubmit, rating, comment }, { headers: { Authorization: `Bearer ${user.accessToken}` } });
            toast.success("Review post ho gaya!");
            setShowReviewModal(false);
            setComment("");
        } catch (err) { toast.error("Review fail ho gaya."); }
    };
    ...
    const submitReport = async () => {
        ...
        const adIdToSubmit = chatData?.chat?.adId?._id || chatData?.chat?.adId;
        try {
            await axios.post(`${API_BASE_URL}/api/reports`, { reportedUserId: otherUser.uid, adId: adIdToSubmit, reason: reportReason, description: reportDesc }, { headers: { Authorization: `Bearer ${user.accessToken}` } });
            toast.success("Report bhej di gayi.");
            setShowReportModal(false);
            setReportDesc("");
        } catch (err) { toast.error("Report fail ho gayi."); }
    };
            ...
            <div className="w-full max-w-4xl bg-white flex flex-col h-screen md:h-[90vh] border shadow-2xl relative md:rounded-3xl overflow-hidden">
                <div className="p-4 border-b flex items-center justify-between bg-white sticky top-0 z-20 shadow-sm">
                    <div className="flex items-center min-w-0">
                        ...
                        <div className="relative">
                            <div className="w-10 h-10 bg-pink-600 rounded-full flex items-center justify-center text-white font-black">{otherUser?.name?.charAt(0).toUpperCase()}</div>
                            {isOnline && <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></span>}
                        </div>
                        <div className="ml-3 truncate">
                            <h3 className="font-bold text-gray-800 text-sm truncate">{otherUser?.name}</h3>
                            <p className="text-[10px] text-gray-500">{isOnline ? "Online Now" : formatLastSeen(lastSeen)}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={() => setShowReviewModal(true)} className="bg-yellow-50 text-yellow-700 px-3 py-1.5 rounded-full text-[10px] font-black uppercase border border-yellow-200"><FaStar /> Rate</button>
                        <button onClick={() => setShowReportModal(true)} className="bg-red-50 text-red-600 px-3 py-1.5 rounded-full text-[10px] font-black uppercase border border-red-100"><FaFlag /> Report</button>
                    </div>
                </div>
                ...
                {ad && (
                    <div className="p-2 border-b bg-gray-50 flex items-center justify-between px-4">
                        <div className="flex items-center min-w-0">
                            <img src={ad.images?.[0]} className="w-12 h-12 rounded-lg object-cover" alt="" />
                            <div className="ml-3 truncate">
                                <h4 className="font-bold text-[11px] text-gray-700">{ad.title}</h4>
                                <p className="text-pink-600 font-black text-xs">Rs {ad.price?.toLocaleString()}</p>
                            </div>
                        </div>
                        <button onClick={() => setAdPreviewModal(ad)} className="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-[10px] font-black uppercase shadow-sm"><FaEye /> View Details</button>
                    </div>
                )}
                    ...
                    {allMessages.map((m, i) => (
                        <div key={i} className={`flex ${m.senderId === user?.uid ? 'justify-end' : 'justify-start'}`}>
                            <div className={`p-3 px-4 rounded-2xl max-w-[80%] text-sm shadow-sm ${m.senderId === user?.uid ? 'bg-pink-600 text-white rounded-tr-none' : 'bg-white text-gray-800 border rounded-tl-none'}`}>
                                ...
                                <p className="text-[8px] mt-1 text-right opacity-70">{new Date(m.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                            </div>
                        </div>
                    ))}
                    <div ref={scrollRef} />
                ...
                <div className="p-4 bg-white border-t flex items-center gap-3">
                    <input className="flex-1 border border-gray-200 rounded-2xl px-5 py-3 text-sm outline-none bg-gray-50 focus:border-pink-500" placeholder="Message likhein..." value={message} onChange={(e) => setMessage(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} />
                    <button onClick={handleSend} className="bg-pink-600 text-white p-4 rounded-2xl shadow-lg active:scale-90"><FaPaperPlane size={18} /></button>
                </div>
                ...
                {showReviewModal && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-[110] p-4">
                        <div className="bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl relative">
                            ...
                            <textarea className="w-full border rounded-2xl p-4 text-sm bg-gray-50 h-28 mb-6" placeholder="Bataein kaisa raha experience..." value={comment} onChange={(e) => setComment(e.target.value)} />
                            <button onClick={submitReview} className="w-full bg-pink-600 text-white py-4 rounded-2xl font-black shadow-xl">SUBMIT FEEDBACK</button>
                        </div>
                    </div>
                )}
                ...
                {showReportModal && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-[110] p-4">
                        <div className="bg-white w-full max-w-sm rounded-3xl p-8 shadow-2xl relative">
                            ...
                            <h3 className="text-2xl font-black text-gray-800 mb-1 text-center">Report User</h3>
                            ...
                            <button onClick={submitReport} className="w-full bg-red-600 text-white py-4 rounded-2xl font-black shadow-xl">SUBMIT REPORT</button>
                        </div>
                    </div>
                )}

                {adPreviewModal && (
                    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[120] p-4 overflow-y-auto">
                        <div className="bg-white rounded-3xl max-w-3xl w-full shadow-2xl relative p-8">
                            <button onClick={() => setAdPreviewModal(null)} className="absolute top-6 right-6 p-2 bg-gray-100 rounded-full hover:bg-red-500 hover:text-white"><FaTimes size={20} /></button>
                            <div className="grid md:grid-cols-2 gap-8 mt-4">
                                <img src={adPreviewModal.images?.[0]} className="w-full h-80 object-cover rounded-3xl shadow-lg border-4 border-gray-50" alt="" />
                                <div className="space-y-4">
                                    <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tighter">{adPreviewModal.title}</h2>
                                    <p className="text-4xl font-black text-pink-600">Rs {adPreviewModal.price?.toLocaleString()}</p>
                                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 italic font-medium text-gray-600">"{adPreviewModal.description}"</div>
                                </div>
                            </div>
                        </div>
                    </div>
