...
// LIVE API URL setup Hostinger ke liye
...
const ChatRoom = ({ user }) => {
    ...
    if (!chatData) {
        return (
            <div className="h-screen flex items-center justify-center font-bold text-pink-600">
                Loading...
            </div>
        );
    }
            ...
            <div className="w-full max-w-4xl bg-white flex flex-col h-screen md:h-[90vh] border shadow-2xl relative md:rounded-3xl overflow-hidden">
                ...
                <div className="p-4 border-b flex items-center justify-between bg-white sticky top-0 z-20 shadow-sm">
                    <div className="flex items-center min-w-0">
                        <FaArrowLeft 
                            className="mr-4 cursor-pointer text-gray-500 hover:text-pink-600 transition-all" 
                            onClick={() => navigate(-1)}
                        />
                    ...
                    <div className="flex items-center gap-2">
                        <button 
                            ...
                            className="flex items-center gap-1.5 bg-yellow-50 text-yellow-700 px-3 py-1.5 rounded-full border border-yellow-200 text-[10px] font-black uppercase hover:bg-yellow-100 transition-all"
                        >
                            <FaStar /> <span className="hidden sm:inline">Rate</span>
                        </button>
                        <button 
                            onClick={() => setShowReportModal(true)} 
                            className="flex items-center gap-1.5 bg-red-50 text-red-600 px-3 py-1.5 rounded-full border border-red-100 text-[10px] font-black uppercase hover:bg-red-100 transition-all"
                        >
                    ...
                    <div className="p-2 border-b bg-gray-50 flex items-center justify-between px-4">
                        <div className="flex items-center min-w-0">
                            ...
                            <div className="ml-3 overflow-hidden">
                                <h4 className="font-bold text-[11px] text-gray-700 truncate">
                                    {ad.title}
                                </h4>
                        ...
                        <button 
                            ...
                            className="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-[10px] font-black uppercase shadow-sm active:scale-95 transition-all"
                        >
                            View Ad
                        </button>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50">
                    {allMessages.map((m, i) => (
                        ...
                        >
                            <div 
                                className={`p-3 px-4 rounded-2xl max-w-[80%] text-sm shadow-sm ${m.senderId === user?.uid ? 'bg-pink-600 text-white rounded-tr-none' : 'bg-white text-gray-800 border rounded-tl-none'}`}
                            >
                ...
                <div className="p-4 bg-white border-t flex items-center gap-3">
                    <input 
                        className="flex-1 border border-gray-200 rounded-2xl px-5 py-3 text-sm outline-none bg-gray-50 focus:border-pink-500 transition-all" 
                    ...
                    <button 
                        onClick={handleSend} 
                        className="bg-pink-600 text-white p-4 rounded-2xl shadow-lg active:scale-90"
                    >
                            ...
                            <div className="flex justify-center gap-3 my-6">
                                {[1, 2, 3, 4, 5].map((s) => (
                                    <FaStar 
                                        key={s} 
                                        onClick={() => setRating(s)} 
                                        className={`text-4xl cursor-pointer transition-all ${rating >= s ? "text-yellow-400" : "text-gray-200"}`} 
                                    />
                                ))}
                            </div>
                            <textarea 
                                className="w-full border rounded-2xl p-4 text-sm outline-none focus:border-pink-500 bg-gray-50 h-28 mb-6" 
                                placeholder="Bataein kaisa raha experience..." 
                                value={comment} 
                                onChange={(e) => setComment(e.target.value)} 
                            />
                            <button 
                                onClick={submitReview} 
                                className="w-full bg-pink-600 text-white py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all"
                            >
                                SUBMIT FEEDBACK
                            </button>
                        </div>
                ...
                {showReportModal && (
                    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-[100] p-4">
                        <div className="bg-white w-full max-w-sm rounded-[2rem] p-8 shadow-2xl relative">
                            ...
                            <select 
                                className="w-full border rounded-2xl p-4 mt-4 text-sm bg-gray-50 outline-none focus:border-red-500 mb-4" 
                                value={reportReason} 
                            ...
                            <textarea 
                                className="w-full border rounded-2xl p-4 text-sm outline-none focus:border-red-500 bg-gray-50 h-28 mb-6" 
                                placeholder="Wajah batayein..." 
                                value={reportDesc} 
                                onChange={(e) => setReportDesc(e.target.value)} 
                            />
                            <button 
                                onClick={submitReport} 
                                className="w-full bg-red-600 text-white py-4 rounded-2xl font-black shadow-xl active:scale-95 transition-all"
                            >
                                SUBMIT REPORT
                            </button>
                        </div>
